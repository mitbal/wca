...
def plot_topic_probability(topics):
    prob = []
    labels = []
    for topic in topics:
...
# Number of lines
print 'Number of messages:', len(chat)
...
plot_word_frequency(dictionary, k)

# Emoji frequency
emoji_unicode = []
emoji_utf8 = []
all_files = glob.glob('emoji/*.jpg')
for f in all_files:
...
translated_text = translate_to_english(chat)
...
with open('translated.txt', 'r') as f:
    for line in f:
        translated_text += line.rstrip()
topics = get_chat_topic(translated_text)
for topic in topics:
    print 'label:', topic['label'], 'probability:', topic['probability']
plot_topic_probability(topics)
